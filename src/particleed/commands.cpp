...
void cmdSavePreset(int n)
{
    ...
    size_t nread = fwrite(&state.ps->info,1,sizeof(hgeParticleSystemInfo),f);
    fclose(f);
#else
...
void cmdLoadPreset(int n)
{
    ...
    size_t nread = fread( &state.ps->info , 1 , sizeof(hgeParticleSystemInfo) , f );
    fclose(f);
#else
	ZeroMemory(filename, _MAX_PATH);
	GetModuleFileName(GetModuleHandle(NULL), filename, _MAX_PATH);
